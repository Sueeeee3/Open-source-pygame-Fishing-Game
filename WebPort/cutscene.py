...
import math
import fish_data
from settings import WIDTH, HEIGHT, S, SP, WHITE
...
class Cutscene:
    ...
    def _draw_dialogue(self, screen, font, text, revealed_chars, gui_rect):
        name_font   = tw(38)
        line_height = name_font.get_linesize() + S(4)
        box_x = gui_rect.x + S(60)
        box_w = gui_rect.width - S(120)
        box_cy = gui_rect.centery

        policeman = text.startswith("@")
        if policeman:
            text = text[1:]
        base_color = (120, 180, 255) if policeman else WHITE
    ...
    def draw(self, screen, assets, font, dt_ms=0):
        ...
        if self.lines and self.current_line < len(self.lines):
            ...
            if self.waiting_after and not line.auto_advance:
                ...
                screen.blit(hint, (gui_rect.right - hint.get_width() - S(20),
                                   gui_rect.bottom - hint.get_height() - S(10)))
        elif self.phase == "wait_inventory":
...
class EndingCutscene(Cutscene):
    ...
    def draw(self, screen, assets, font, dt_ms=0):
        ...
        if self.phase in ("reaction", "police", "footsteps"):
            ...
            screen.blit(dot_surf, dot_surf.get_rect(center=(WIDTH // 2, HEIGHT // 2 - S(40))))
            ...
            for rect, label in [(again_rect, "Try Again"), (homeless_rect, "Go back to your day job")]:
                ...
                ls = btn_font.render(label, True, WHITE)
            ...
            if self.phase == "homeless_text":
                msg = tw(72).render("You became homeless.", True, WHITE)
                screen.blit(msg, msg.get_rect(center=(WIDTH // 2, HEIGHT // 2)))
            elif self.phase == "credits":
                credit_font = tw(40)
                line_spacing = S(56)
                total_h = len(self.credits_lines) * line_spacing
                start_y = HEIGHT // 2 - total_h // 2
                for i, line in enumerate(self.credits_lines):
                    if line:
                        cs = credit_font.render(line, True, (200, 200, 200))
                        screen.blit(cs, cs.get_rect(center=(WIDTH // 2, start_y + i * line_spacing)))

...
import math
import fish_data
from settings import WIDTH, HEIGHT, S, SP
...
class Fishpedia:
    def __init__(self, assets):
        ...
        self.panel_w = int(WIDTH * 0.88)
        self.panel_h = int(HEIGHT * 0.88)
        self.panel_x = (WIDTH  - self.panel_w) // 2
        self.panel_y = (HEIGHT - self.panel_h) // 2
        self.cols     = 4
        self.img_size = S(150)
    ...
    def handle_input(self, event, inventory):
        ...
        if event.type == pygame.MOUSEWHEEL and self.open:
            ...
            for cat in CATEGORY_ORDER:
                ...
                if fish:
                    total_h += S(65) + math.ceil(len(fish) / self.cols) * self.cell_h + S(20)
    ...
    def draw(self, screen, assets):
        ...
        current_y = self.panel_y + S(20) - scroll_offset
        ...
        for category in CATEGORY_ORDER:
            ...
            if self.panel_y - self.cell_h < current_y < self.panel_y + self.panel_h:
                hx = self.panel_x + S(20)
                header = f50.render(CATEGORY_LABELS[category], True, (80, 50, 15))
                screen.blit(header, (hx, current_y))
                pygame.draw.line(screen, BORDER_COLOR,
                                 (hx, current_y + header.get_height() + S(3)),
                                 (hx + header.get_width(), current_y + header.get_height() + S(3)), 2)
            current_y += S(65)
            ...
            for i, (name, info) in enumerate(fish_in_cat):
                ...
                if not is_caught:
                    ...
                    screen.blit(dark, (img_x, y + S(8)))
                else:
                    screen.blit(fish_img, (img_x, y + S(8)))

                text_y   = y + self.img_size + S(16)
                text_col = (60, 35, 10) if is_caught else (130, 110, 85)
                display_name = name.replace("_", " ") if is_caught else "???"

                screen.blit(f30.render(display_name, True, text_col), (x + S(8), text_y))
                text_y += f30h + S(5)
                ...
                if is_caught:
                    ...
                    for word in DESCRIPTIONS.get(name, "A mysterious fish.").split():
                        test = line + word + " "
                        if f16.size(test)[0] > card_w - S(16):
                            surf = f16.render(line.strip(), True, (80, 60, 35))
                            screen.blit(surf, (x + S(8), text_y))
                            text_y += surf.get_height() + S(2)
                            line = word + " "
                        else:
                            line = test
                    if line.strip():
                    ...
                    screen.blit(size_surf, (x + S(8), y + card_h - size_surf.get_height() - S(8)))
                else:
                    screen.blit(f20.render("???", True, (150, 130, 100)), (x + S(8), text_y))

            current_y += math.ceil(len(fish_in_cat) / self.cols) * self.cell_h + S(20)
        ...
        if total_height > self.panel_h:
            ...
            sb_x  = self.panel_x + self.panel_w - S(16)
            pygame.draw.rect(screen, (180, 155, 110), (sb_x, self.panel_y, S(11), self.panel_h), border_radius=5)
            pygame.draw.rect(screen, (100, 70,  30),  (sb_x, bar_y,        S(11), bar_h),        border_radius=5)
